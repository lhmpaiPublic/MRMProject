...
BOOL CXLAutomation::PasteStringToWorksheet(CString pDataBuffer)
{
	...
	HANDLE hMem = GlobalAlloc(GMEM_MOVEABLE|GMEM_DDESHARE, 24);
	  ...
	  if (OpenClipboard(NULL) )
	  {
		...
		EmptyClipboard();
		hClipMem = GlobalAlloc(GMEM_DDESHARE, sizeof(TCHAR)*(nBuffSize+1));
		buffer = (TCHAR*)GlobalLock(hClipMem);
		memset(buffer, NULL, sizeof(TCHAR)*(nBuffSize+1));
		memcpy(buffer, pDataBuffer, sizeof(TCHAR)*nBuffSize);
